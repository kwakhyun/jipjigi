...
export function recordProductEvent(input: unknown, userId: string | null) {
  ...
  const assignment = userId
    ? db.prepare("SELECT experiment_key AS experimentKey, variant FROM experiment_assignments WHERE user_id = ? ORDER BY assigned_at DESC LIMIT 1").get(userId) as { experimentKey: string; variant: string } | undefined
    : undefined;
...
export function recordServerProductEvent(
  ...
  const assignment = db
    ...
    .get(userId) as { experimentKey: string; variant: string } | undefined;

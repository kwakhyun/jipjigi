...
describe("CRM 공급자 웹훅", () => {
  it("전달, 갱신 응답과 수신 해제를 계약 타임라인과 제품 이벤트에 저장한다", async () => {
    const result = runOperation("owner-1", { type: "start_renewal", leaseId: "lease-seongsu-501" });
    ...
    const lease = getDatabase().prepare(
      "SELECT renewal_status AS renewalStatus, contact_consent AS contactConsent FROM leases WHERE id = ?",
    ).get("lease-seongsu-501") as { renewalStatus: string; contactConsent: number };
    ...
    ).get("lease-seongsu-501") as { response: string };
    expect(timeline.response).toBe("agreed");
    const eventNames = getDatabase().prepare(
      "SELECT name FROM product_events WHERE user_id = ? ORDER BY occurred_at",
    ).all("owner-1") as Array<{ name: string }>;
  ...
  it("클라이언트가 보낸 값 대신 서버 릴리스 버전을 저장한다", () => {
    ...
    ).get(eventId) as { releaseVersion: string; userSegment: string };

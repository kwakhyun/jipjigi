...
async function leaseIdForDispatch(database: AppDatabase, dispatch: DispatchRecord) {
    ...
    return await database.transaction(async (db) => {
      const dispatch = await db.prepare(
        `SELECT id, user_id AS userId, entity_type AS entityType, entity_id AS entityId,
         ...
         FROM message_dispatches WHERE provider_message_id = ?${event.status === "opted_out" ? "" : " FOR UPDATE"}`,
      ...
      if (event.status === "opted_out") {
        const leaseId = await leaseIdForDispatch(db, dispatch);
        if (!leaseId) return NextResponse.json({ error: "계약을 찾을 수 없습니다." }, { status: 404 });
        await db.prepare("SELECT id FROM leases WHERE id = ? FOR UPDATE").get(leaseId);
        ...
        if (inserted.changes > 0) {
          await writeAudit(dispatch.userId, "crm_opted_out", "lease", leaseId, { channel: dispatch.channel, messageId: dispatch.id }, db);

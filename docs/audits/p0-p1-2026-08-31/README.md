...
이 문서는 `a335fb3`까지의 과거 감사 기록입니다. 이후 계약 조회, 데모 전환, 이벤트 신뢰 경계에서 추가 문제가 확인되어 수정했습니다. 현재 상태는 [후속 개선 기록](../2026-08-31-trust-and-consistency.md)을 확인하세요. 아래 테스트 수와 스크린샷은 당시 근거로 보존합니다.
...
- 운영자 원격 로그인도 승인 검토에서 차단되어 실행하지 않았습니다. 역할 변경과 사용자 삭제 후 세션 거부는 로컬 테스트로 검증했습니다.
- 첫 Production 빌드에서 플랫폼의 `VERCEL_ENV=production`이 테스트에 전달되는 차이를 확인했습니다. Vitest에 테스트 환경과 빈 외부 저장소 자격 증명을 명시했고, GitHub CI도 이 플랫폼 표시를 주입해 회귀를 재현하도록 보강했습니다. 운영 런타임의 저장소 필수 조건은 유지합니다.

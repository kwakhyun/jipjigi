...
2026년 8월 31일 Query 동기화 적용 후 빌드 기준 결과입니다.
...
| `/app` | 176.7KiB | 185KiB |
| `/app/contracts` | 166.1KiB | 170KiB |
| `/app/growth` | 150.8KiB | 170KiB |
| `/app/ledger` | 166.6KiB | 170KiB |
| `/app/maintenance` | 166.9KiB | 170KiB |
| `/app/messages` | 167.6KiB | 170KiB |
| `/app/settings` | 161.6KiB | 170KiB |

목록의 Query 도입으로 초기 전송량은 약 13~14KiB 늘었습니다. 대신 화면 간 서버 상태 공유, 중복 조회 제거와 오류 복구를 얻습니다. Query 공급자를 운영 데이터 route group에, Jotai를 홈에 한정해 공개 경로와 그로스 관제는 이전 크기를 유지했습니다. 기존 CI 예산을 올리지 않았습니다.

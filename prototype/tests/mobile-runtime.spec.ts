...
test("navigation does not reveal the software keyboard", async ({ page }) => {
  await page.goto("/");
  const keyboard = page.getByTestId("keyboard-dock");

  await expect(keyboard).toHaveAttribute("data-visible", "false");
  await page.getByRole("button", { name: "임대 장부" }).click();
...
test("each main tab opens at the top of its own screen", async ({ page }) => {
  await page.goto("/");
  ...
  await page.getByRole("button", { name: "임대 장부" }).click();
  await expect(page.getByRole("heading", { name: "임대 장부" })).toBeVisible();
  expect(await page.getByTestId("mobile-scroll").evaluate((element) => element.scrollTop)).toBe(0);
});
...
test("keyboard and its attached footer dismiss on the same transition", async ({ page }) => {
  ...
  await expect(keyboard).toHaveCSS("transform", "matrix(1, 0, 0, 1, 0, 0)");
